...
class QueryProcessor:
    ...
    def rerank_results(
        ...
        try:
            ...
            for result in results:
                ...
                # 计算多样性分数
                diversity_score = self._calculate_diversity_score(result, reranked_results)
            ...
            logger.info(f"重排序完成，分数范围: {reranked_results[0]['rerank_score']:.3f} - {reranked_results[-1]['rerank_score']:.3f}")
    ...
    def _calculate_diversity_score(
        ...
        try:
            ...
            for existing in existing_results:
                ...
                # 简单的内容相似性计算（基于共同词汇）
                content_sim = self._calculate_text_similarity(current_content, existing_content)
    ...
    def _calculate_content_quality_score(self, result: Dict[str, Any]) -> float:
        ...
        try:
            ...
            # 水文专业术语
            hydro_terms = ['模型', '参数', '径流', '降水', '蒸发', '土壤', '含水量', '流域']
    ...
    def filter_results(
        ...
        try:
            ...
            if min_score is not None:
                score_key = 'rerank_score' if 'rerank_score' in filtered_results[0] else 'score'
                filtered_results = [
                    r for r in filtered_results
                    if r.get(score_key, 0) >= min_score
                ]
    ...
    def _extract_keywords(self, text: str, max_keywords: int = 5) -> List[str]:
        ...
        try:
            ...
            # 水文领域关键词
            hydro_keywords = [
                'GR4J', 'XAJ', '新安江', '模型', '参数', '径流', '降水',
                '蒸发', '土壤', '含水量', '流域', '率定', '校准', '敏感性',
                '目标函数', '纳什效率', 'NSE', '相关系数', '均方根误差'
            ]
            ...
            word_freq = {}
            for word in words:
                if len(word) > 2 and word not in ['的', '和', '或', '但', '因为', '所以']:
                    word_freq[word] = word_freq.get(word, 0) + 1

            # 获取高频词
            high_freq_words = sorted(word_freq.items(), key=lambda x: x[1], reverse=True)
