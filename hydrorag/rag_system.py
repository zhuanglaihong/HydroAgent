...
class RAGSystem:
    ...
    def _initialize_system(self):
        ...
        try:
            ...
            self.is_initialized = (
                ...
                self.knowledge_updater is not None
            )
            
            if self.is_initialized:
                logger.info(f"RAG系统所有组件初始化成功，使用 {self.vector_backend} 向量后端")
            else:
    ...
    def query(
        ...
        try:
            ...
            logger.info(f"查询: {query_text[:100]}..., top_k={top_k}, threshold={score_threshold}, 后端={self.vector_backend}")
            ...
            if self.vector_backend == "faiss":
                ...
                return self._query_with_faiss(
                    queries_to_search, top_k, score_threshold,
                    include_metadata, enable_rerank
                )
            else:
                # 传统Chroma后端
    ...
    def _query_with_chroma(
        ...
        try:
            ...
            if enable_rerank and self.query_processor:
                reranked_results = self.query_processor.rerank_results(queries[0], unique_results)
            else:
    ...
    def _convert_faiss_results(self, raw_result: Dict[str, Any]) -> List[Dict[str, Any]]:
        """转换FAISS结果为标准格式"""
        try:
            results = []
    ...
    def _rank_fusion(self, results: List[Dict[str, Any]], queries: List[str]) -> List[Dict[str, Any]]:
        """查询结果融合排序"""
        try:
            ...
            for result in results:
                ...
                if content in content_scores:
                    content_scores[content]["score"] = max(content_scores[content]["score"], combined_score)
                else:
            ...
            # 按分数排序
            fused_results = sorted(content_scores.values(), key=lambda x: x["score"], reverse=True)
    ...
    def create_knowledge_backup(self, backup_name: Optional[str] = None) -> Dict[str, Any]:
        """
        创建知识库备份
            ...
            status = {
                ...
                "components": {
                    ...
                    "knowledge_updater": {
                        ...
                        "status": self.knowledge_updater.get_status() if self.knowledge_updater else None
                    }
                },
                "features": {
    ...
    def setup_from_raw_documents(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (processed_docs and
                ...
                logger.info(f"发现已存在的{self.vector_backend}向量库和处理后的文档，跳过处理")
                return {
                    "status": "success",
...
def quick_setup(documents_dir: str = "./documents", use_faiss: bool = True) -> RAGSystem:
    """
    快速设置RAG系统
