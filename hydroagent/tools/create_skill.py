...
_SKILL_TOOL_PROMPT = '''You are writing a HydroAgent tool module for a new Skill package.
...
_SKILL_MD_PROMPT = '''You are writing a skill.md guide for a new HydroAgent Skill.
...
def create_skill(
    ...
    Generates a complete Skill directory under hydroagent/skills/<skill_name>/ with:
    ...
    if load_test.returncode != 0:
        ...
        from hydroagent.error_kb import ErrorKnowledgeBase
...
def _find_similar_tools(skill_name: str, description: str,
    ...
    """
    try:
        from hydroagent.tools import discover_tools, _TOOL_META
    except ImportError:
        return []
...
def _validate_imports(code: str) -> list[str]:
    ...
    Skips imports of stdlib and the project's own hydroagent package.
    """
    SKIP_PREFIXES = ("hydroagent", "pathlib", "logging", "typing", "json", "os",
