...
Date: 2026-02-08
Description: CLI entry point for HydroAgent.

Usage:
  python -m hydroagent                      # user mode (interactive)
  python -m hydroagent "rate GR4J basin X"  # user mode (single query)
  python -m hydroagent --dev                # developer mode (interactive)
  python -m hydroagent --dev "query"        # developer mode (single query)
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="HydroAgent - LLM-driven hydrological model calibration agent",
...
Examples:
  ...
  python -m hydroagent --dev "批量率定三个流域"
        """,
    )
    ...
    agent = HydroAgent(config_path=args.config, workspace=workspace, ui=ui)
...
def _launch_server(port: int = 7860, workspace: str | None = None):
    ...
    from hydroagent.interface.server import create_app
    ...
    print(f"HydroAgent Web Server -> {url}")
...
def _setup_server_logging(ws_path: Path):
    ...
    Writes to logs/hydroagent_server_YYYYMMDD_HHMMSS.log in the workspace.
    ...
    log_path = logs_dir / f"hydroagent_server_{ts}.log"
...
def _print_banner(ui, agent):
    ...
    from hydroagent.tools import discover_tools
...
def _setup_logging(dev: bool, log_file: str | None, console=None):
    ...
    if log_file:
        ...
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        log_path = logs_dir / f"hydroagent_{ts}.log"
...
def _maybe_run_setup_wizard(ui, explicit_config_path: str | None):
    ...
    检测逻辑：
    - 先用当前配置源（含 ~/.hydroagent/config.json + hydroagent legacy）加载
    ...
    from hydroagent.utils.setup_wizard import needs_setup, run_wizard
