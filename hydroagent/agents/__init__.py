...
Scans two layers (lower priority first, higher overrides):
  1. Built-in:      hydroagent/agents/*.md
  2. Project-level: <workspace>/.hydroagent/agents/*.md
...
class AgentRegistry:
    ...
    def __init__(self, workspace: Path | None = None):
        ...
        # Layer 1: built-in agents shipped with hydroagent
        self._scan(Path(__file__).parent)
        # Layer 2: project-level overrides (higher priority)
        if workspace:
            self._scan(Path(workspace) / ".hydroagent" / "agents")
