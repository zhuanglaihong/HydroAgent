...
保存到 ~/.hydroagent/config.json，后续启动直接读取，无需重复配置。
"""
...
# 配置文件保存在项目根目录（hydroagent 包的上一级）
USER_CONFIG_PATH = Path(__file__).parent.parent / "hydroagent_config.json"
...
def run_wizard(console: Console) -> dict:
    ...
    向导结束后自动保存到 ~/.hydroagent/config.json。
    """
    console.print()
    console.print(Panel(
        Text("HydroAgent 首次配置向导", style="bold cyan", justify="center"),
...
def _save_config(cfg: dict, console: Console) -> Path:
    """保存配置到 ~/.hydroagent/config.json，API Key 提示是否也保存到环境变量。"""
