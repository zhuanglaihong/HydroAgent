...
7. **工具创建** - 当现有工具无法满足需求时，自动生成新工具包装脚本（`create_tool`），支持集成任意 Python 包
...
当用户需要的功能不在现有工具中时，使用 `create_tool` 创建新工具：
- 告诉它工具名称、功能描述、要包装的 Python 包
- 新工具会自动写入 `hydroclaw/tools/` 并立即可用
- 适用于集成新的水文分析包、GitHub 代码仓等
