...
def create_app(workspace: str = ".") -> FastAPI:
    ...
    async def api_tools():
        ...
        try:
            from hydroclaw.tools import get_tool_registry_info
        ...
        CORE_HYDRO = [
            {
                ...
                "detail": "提供 calibrate / evaluate / simulate 三类操作。HydroClaw 的核心率定能力由本包提供。支持 spotpy 接口的所有优化算法。",
            },
            {
                "name": "hydrodataset",
                ...
                "detail": "基于 AquaFetch 自动下载并缓存为 NetCDF。通过 ~/hydro_setting.yml 配置数据路径，HydroClaw 启动时自动同步。",
    ...
    async def api_datasets():
        ...
        override_file = ws_path / "hydroclaw_config_override.json"
        custom = []
    ...
    async def api_config_save(body: _ConfigPatch):
        """Save config overrides to workspace hydroclaw_config_override.json."""
        override_file = ws_path / "hydroclaw_config_override.json"
        override = {}
        if override_file.exists():
            try:
        ...
        if body.api_key is not None and body.api_key != "" and "***" not in body.api_key:
            ...
            from hydroclaw.config import _deep_merge
            proj_cfg_path = Path(__file__).parent.parent.parent / "hydroclaw_config.json"
            proj_cfg = {}
