"""
Meta-tool: register a single .py file's public functions as HydroClaw tools.
...
def add_local_tool(
    ...
    """Register all public functions in a .py file as HydroClaw tools.
    ...
    mod_key = f"_hydroclaw_plugin_{plugin_name}_tool"
    registered: list[str] = []
    errors: list[str] = []
    ...
    try:
        ...
        from hydroclaw.tools import _register_tool, PRIORITY_DYNAMIC, reload_tools
        # Force reload so the new functions appear in schema
        from hydroclaw.tools import _TOOLS, _TOOL_META
