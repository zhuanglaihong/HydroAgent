...
HydroClaw 通过 [hydrodataset](https://github.com/OuyangWenyu/hydrodataset) 包访问水文数据集。
...
hydrodataset 从用户主目录的 `~/hydro_setting.yml` 读取数据路径，与 HydroClaw 的 `configs/private.py` 中的 `DATASET_DIR` 是**独立的两套配置**。
...
**HydroClaw 会根据 `configs/private.py` 中的 `DATASET_DIR` 自动生成此文件，无需手动编辑。**

`DATASET_DIR` 应指向**具体数据集目录**（如 `D:\data\CAMELS_US`），HydroClaw 自动取其父目录（`D:\data`）写入 `datasets-origin`：
...
- HydroClaw 的 `validate_basin` 工具会自动识别当前数据集并做对应的 ID 验证
