...
HydroClaw 是一个有**原则**（system.md）、有**技能**（skills）、有**参考**（knowledge）、有**记忆**（memory）的水文专家 Agent，
...
**文件**：`hydroclaw/skills/calibration/skill.md` 异常处理部分新增：
...
| 情况 | 查阅文件 |
|------|---------|
| 报错原因不明、不知道如何恢复 | `hydroclaw/knowledge/failure_modes.md` |
| 工具连续报同一个错 | `hydroclaw/knowledge/tool_error_kb.md` |
| 不确定数据集支持哪些流域或变量 | `hydroclaw/knowledge/datasets.md` |
| 不确定模型参数的物理含义 | `hydroclaw/knowledge/model_parameters.md` |
```

**文件**：`hydroclaw/skills/llm_calibration/skill.md` 异常处理部分同样新增上述表格。
...
**文件**：`hydroclaw/skills/calibration/skill.md` 和 `llm_calibration/skill.md` 的结尾加：
...
**文件**：`hydroclaw/skills/calibration/skill.md` 的"任务开始"部分新增：
...
验证：
  python -m hydroclaw "率定XAJ，流域12025000"
