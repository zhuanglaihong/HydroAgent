...
def organize_stations_df(stations, basin):
    ...
    for poly_id in np.unique(nearest_gdf["index_right"].to_numpy()):
        ...
        if len(sta_ids) >= 2:
            ...
            pp_comp_df = pp_comp_df.resample("1h", origin="epoch").interpolate()[
                tm_min:tm_max
            ]
            rr_comp_df = rr_comp_df.resample("1h", origin="epoch").interpolate()[
                tm_min:tm_max
            ]
...
def organize_rain_and_flow(dfs):
    ...
    rr_comp_df = (
        pd.concat([river_comp_df, rsvr_comp_df]).groupby(level=0).agg({"q": "mean"})
    )
