...
try:
    from config import (
        LLM_USE_API_FIRST, LLM_API_MODEL_NAME, LLM_API_TIMEOUT, LLM_API_MAX_RETRIES,
        LLM_FALLBACK_MODEL, LLM_FALLBACK_TIMEOUT
    )
...
class QwenAPIClient(BaseLLMClient):
    ...
    def chat(self, messages: List[LLMMessage], **kwargs) -> LLMResponse:
        ...
        try:
            ...
            return LLMResponse(
                success=False,
                error=f"千问API调用超时 (>{LLM_API_TIMEOUT}s)"
            )
        except Exception as e:
...
class IntelligentLLMClient(BaseLLMClient):
    ...
    def __init__(self, enable_debug: bool = False):
        ...
        self.logger.info(f"智能LLM客户端初始化完成 - 推理API可用: {self.reasoning_client is not None}")

    def chat(self, messages: List[LLMMessage], task_type: str = "reasoning", **kwargs) -> LLMResponse:
    ...
    def _handle_reasoning_task(self, messages: List[LLMMessage], **kwargs) -> LLMResponse:
    ...
    def _fallback_to_local(self, messages: List[LLMMessage], task_type: str, **kwargs) -> LLMResponse:
        """降级到本地模型"""
        ...
        if not self.fallback_client.is_available():
            return LLMResponse(
                success=False,
                error="API调用失败且本地Ollama服务不可用，无法处理任务"
            )
