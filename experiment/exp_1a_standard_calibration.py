...
TEST_QUERIES = [
    ...
    # 1. 基础率定 - 最简单的查询（仅模型+流域）
    "率定GR4J模型，流域01539000",

    # 2. 指定算法 - 率定 + 指定算法
    "率定XAJ模型，流域02070000，使用SCE-UA算法",

    # 3. 指定算法参数 - 率定 + 算法 + 迭代轮数
    "率定GR4J模型，流域02177000，使用SCE-UA算法，迭代500轮",

    # 4. 指定时间期 - 率定 + 训练期 + 测试期
    "率定XAJ模型，流域03346000，训练期1990-2000，测试期2005-2015",

    # 5. 指定warmup - 率定 + warmup期
    "率定GR4J模型，流域03500000，warmup期365天",

    # 6. 全参数率定 - 包含所有可能的参数（最完整的模板）
    ...
    # 11. 扩展分析 - 多种分析任务
    "率定GR4J模型，流域01539000，完成后计算径流系数并画FDC曲线",
    ...
    # 15. 复杂组合 - 时间期 + 迭代优化
    "率定XAJ模型，流域03500000，训练期1990-2005，测试期2010-2015，如果NSE低于0.7则调整参数范围重新率定",
]
...
def main():
    ...
    print(f"")
    print(f"   - 其他任务类型: 9 个")
    print(f"     7. 评估任务 (evaluation)")
    print(f"     8. 模拟任务 (simulation)")
    print(f"     9-11. 扩展分析 (extended_analysis)")
    print(f"     12-13. 迭代优化 (iterative_optimization)")
    print(f"     14-15. 复杂组合")
    print(f"")
    print(f"🔧 配置:")
