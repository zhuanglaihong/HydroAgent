...
class KnowledgeFragment:
    ...
    source: str                     # 来源
    score: float                    # 相似度得分
    fragment_type: str = "general"  # 片段类型：general, tool_doc, workflow_case, background
...
class CoTRAGEngine:
    ...
    def __init__(self, rag_system=None, ollama_client=None, config: Optional[Dict[str, Any]] = None):
        """
        初始化推理引擎
...
"""
    ...
    def generate_reasoning_plan(self, intent_result: IntentResult) -> Tuple[RAGRetrievalResult, CoTReasoningResult]:
        """
        生成推理计划
        ...
        try:
            ...
            if hasattr(self.rag_system, 'query'):
                results = self.rag_system.query(
                    query, 
                    k=self.config.get("rag_retrieval_k", 5),
    ...
    def _retrieve_fallback_knowledge(self, query: str, intent_result: IntentResult) -> List[KnowledgeFragment]:
        """回退知识检索"""
        # 提供基础的知识片段作为回退
        fallback_knowledge = {
            "model_calibration": [
    ...
    def _deduplicate_and_rank_fragments(self, fragments: List[KnowledgeFragment]) -> List[KnowledgeFragment]:
        """去重和排序知识片段"""
        # 简单的去重：基于内容的前100个字符
        seen_contents = set()
        unique_fragments = []
    ...
    def _build_cot_prompt(self, intent_result: IntentResult, rag_result: RAGRetrievalResult) -> str:
        """构建CoT推理提示词"""
        # 构建知识上下文
...
"""
    ...
    def _parse_cot_response(self, response: str) -> Tuple[List[CoTReasoningStep], str]:
        ...
        try:
            # 尝试提取推理步骤
            reasoning_pattern = r'(\d+)\.\s*\*\*([^*]+)\*\*([^0-9]+?)(?=\d+\.\s*\*\*|\{|$)'
...
def create_cot_rag_engine(rag_system=None, ollama_client=None, config=None) -> CoTRAGEngine:
    """创建CoT+RAG推理引擎实例"""
    return CoTRAGEngine(rag_system=rag_system, ollama_client=ollama_client, config=config)
