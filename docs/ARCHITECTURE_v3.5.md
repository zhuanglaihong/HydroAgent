...
**Version**: 3.5 (All Experiments Ready)
**Status**: Core Implementation Complete, All Experiments Validated
...
| **Phase 5: 端到端验证** | ✅完成 | 2025-01-25 | 实验1-5全面验证完成 |

### 1.2 实验验证状态

| 实验 | 状态 | 测试结果 | 备注 |
|------|------|---------|------|
| **实验1：标准流域验证** | ✅通过 | NSE=0.68 (Good) | 已修复JSON解析和config使用问题 |
| **实验2A：全信息率定** | ✅通过 | 参数提取正常 | - |
| **实验2B：缺省信息补全** | ✅通过 | 信息自动补全 | - |
| **实验2C：自定义数据** | ✅通过 | 自定义路径支持 | - |
| **实验3：参数自适应优化** | ✅通过 | 动态参数范围调整 | 支持智能停止条件和range_scale调整 |
| **实验4：代码生成扩展** | ✅通过 | 双LLM代码生成 | 通用LLM + 代码专用LLM架构 |
| **实验5：稳定性验证** | ✅通过 | 统计分析正常 | - |
...
- ✅ **智能参数范围调整**（2025-01-25完善）
  - 动态调整range_scale（60% → 15%）
  - 以最优参数为中心缩小搜索范围
  - 智能停止条件（NSE达标、无改善、迭代上限）
...
- ✅ **双LLM架构（通用LLM + 代码LLM）**（2025-01-25完善）
  - API模式: qwen-turbo (思考) + qwen-coder-turbo (代码生成)
  - Ollama模式: qwen3:8b (思考) + deepseek-coder:6.7b (代码生成)

### 1.4 最新更新（v3.5 - 2025-01-25）

#### 实验脚本重构
- **统一BaseExperiment类**: 所有实验使用统一基类
- **动态输出目录**: 自动使用实验名称（不再硬编码"exp1"）
- **终端显示优化**: 显示正确的实验名称

#### 自适应迭代优化（实验3）
- **动态range_scale调整**: 60% → 15%（以最佳参数为中心缩小搜索范围）
- **智能停止条件**:
  - NSE达到优秀阈值（>0.75）
  - 连续无改善（tolerance < 0.001）
  - 达到迭代上限
- **自动生成参数范围文件**: 基于上一轮最优参数生成新的YAML文件

#### 双LLM代码生成（实验4）
- **分离思考与代码生成**: 通用LLM负责分析，代码专用LLM负责生成
- **API模式**: qwen-turbo (通用) + qwen-coder-turbo (代码生成)
- **Ollama模式**: qwen3:8b (通用) + deepseek-coder:6.7b (代码生成)
- **支持任务**: 径流系数、FDC曲线、自定义水文指标等

### 1.5 历史修复记录

#### 实验4: Custom Analysis任务路由问题（2025-01-24）
**问题**：
...
**系统行为**（v3.5优化）
1. IntentAgent → 识别"迭代优化"任务
2. TaskPlanner → 创建单一iterative任务
3. RunnerAgent → 内部循环处理多轮迭代
   - **初始轮**: 使用默认参数范围（range_scale=60%）
   - **后续轮**: 动态调整参数范围（range_scale=15%，以上轮最优参数为中心）
   - **智能停止条件**:
     - NSE达到优秀阈值（≥0.75）
     - 连续无改善（tolerance < 0.001）
     - 达到最大迭代轮数（max_iterations）
4. DeveloperAgent → 分析所有轮次结果，对比NSE提升趋势

**关键改进**（v3.5）：
- 取消了两阶段固定架构，改为灵活的多轮迭代
- 参数范围自动收缩（60% → 15%），精准搜索最优解
- 自动生成参数范围YAML文件（`param_range_iter{N}.yaml`）
...
**系统行为**（v3.5优化）
1. IntentAgent → 识别"扩展分析"任务，提取需求（runoff_coefficient, FDC）
2. TaskPlanner → 拆解为：
   - Task 1: 标准率定（如需要）
   - Task 2: custom_analysis任务（最小化配置）
3. RunnerAgent → 执行Task 1（可选）
4. **DeveloperAgent（双LLM架构）** → 代码生成：
   - **步骤1**: 使用**通用LLM**（qwen-turbo/qwen3:8b）进行任务理解和分析
   - **步骤2**: 使用**代码专用LLM**（qwen-coder-turbo/deepseek-coder:6.7b）生成Python代码
   - **输出**: 完整可运行的脚本（`calc_runoff_coef.py`, `plot_fdc.py`）
5. RunnerAgent → 执行生成的脚本（可选）
6. DeveloperAgent → 汇总所有结果

**关键改进**（v3.5）：
- **分离思考与代码生成**: 通用模型擅长理解需求，代码模型擅长生成代码
- **支持API和Ollama**:
  - API: qwen-turbo + qwen-coder-turbo
  - Ollama: qwen3:8b + deepseek-coder:6.7b
- **完整的代码生成**: 包含type hints、注释、错误处理
...
class DeveloperAgent(BaseAgent):
    ...
    def _llm_generate_script(self, task: str, input_files: List[str], output_file: str) -> str:
        """使用双LLM架构生成Python脚本（v3.5）"""

        # 🆕 步骤1: 使用通用LLM进行任务理解和分析
        analysis_prompt = f"""
        请分析以下代码生成任务：
        ...
        请说明：
        1. 需要使用什么库（xarray/pandas/matplotlib等）
        2. 关键计算步骤
        3. 注意事项
        """

        analysis = self.llm_interface.chat([  # 使用通用LLM (qwen-turbo/qwen3:8b)
            {"role": "system", "content": "你是水文分析专家，帮助理解任务需求"},
        ...
        response = self.code_llm_interface.chat([  # 🆕 使用代码专用LLM (qwen-coder-turbo/deepseek-coder)
            {"role": "system", "content": "你是Python代码生成专家"},
...
TaskPlanner → 创建单一iterative任务

RunnerAgent → 多轮迭代循环:
  第1轮:
    - 使用默认参数范围（range_scale=60%）
    - 执行率定，获取最优参数
    - 检查停止条件（NSE达标/无改善/达到上限）

  第2-N轮（如未满足停止条件）:
    - 动态生成新参数范围（range_scale=15%，以上轮最优参数为中心）
    - 生成参数范围文件 param_range_iter{N}.yaml
    - 执行率定，获取新的最优参数
    - 检查停止条件

DeveloperAgent → 分析所有轮次结果，对比NSE提升趋势

关键改进（v3.5）:
- 取消固定两阶段，改为灵活多轮迭代
- 参数范围自动收缩（60% → 15%）
- 智能停止条件（NSE≥0.75 或 连续无改善）
```
...
TaskPlanner → 拆解为Task1(标准率定, 可选) + Task2(custom_analysis)

Task 1 (可选):
  InterpreterAgent → config
  RunnerAgent → 执行率定

Task 2 (custom_analysis):
  InterpreterAgent → 生成最小化配置（只包含task_metadata）

  DeveloperAgent (双LLM架构):
    步骤1: 使用通用LLM (qwen-turbo/qwen3:8b)
      - 分析任务需求
      - 确定所需的库和计算步骤

    步骤2: 使用代码专用LLM (qwen-coder-turbo/deepseek-coder:6.7b)
      - 基于分析结果生成完整Python代码
      - 包含type hints、注释、错误处理
      - 输出: calc_runoff_coef.py, plot_fdc.py

  RunnerAgent → 执行生成的脚本（可选）
  DeveloperAgent → 汇总所有结果

关键改进（v3.5）:
- 双LLM架构：分离思考与代码生成
- custom_analysis最小化配置
- 支持API和Ollama两种后端
```
...
3. **双LLM代码生成（实验4）**
   - 分离思考与代码生成
   - 通用LLM（qwen-turbo/qwen3:8b）负责任务理解
   - 代码专用LLM（qwen-coder-turbo/deepseek-coder:6.7b）负责代码生成
   - 支持API和Ollama两种后端
