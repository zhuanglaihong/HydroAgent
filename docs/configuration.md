...
>
> HydroAgent 使用的 [AquaFetch](https://github.com/hyex-research/AquaFetch) 会在传入路径后**自动拼接数据集类名**（如 `CAMELS_US`）。
...
>
> **`~/hydro_setting.yml` 无需手动配置。** HydroAgent 启动时会根据 `DATASET_DIR` 和 `CACHE_DIR` 自动生成/更新此文件。
...
`build_hydromodel_config()` 将 HydroAgent 参数转换为 hydromodel 格式：
...
```
~/.hydroagent/plugins.json              # 全局（所有工作区共享）
<workspace>/.hydroagent/plugins.json    # 本地（本项目覆盖全局）
```
