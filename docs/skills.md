...
Skill 是 HydroAgent 的核心扩展单元，每个 Skill 是一个子目录，包含两部分：
...
委派任务给指定的子代理（在 `hydroagent/agents/` 或 `.hydroagent/agents/` 中定义）。
...
注册本地目录包（包根目录需有 `hydroagent_adapter.py`）。
...
# -> 写入 <workspace>/.hydroagent/plugins.json，调用 reload_adapters()
```
