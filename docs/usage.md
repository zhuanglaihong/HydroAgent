...
# 指定配置和工作目录
python -m hydroagent -c my_config.json -w results/exp_A "率定GR4J模型，流域12025000"
```
...
```bash
python -m hydroagent --server          # 启动 FastAPI Web 服务（默认端口 7860）
python -m hydroagent --server --port 8080  # 自定义端口
```
...
HydroAgent 自动执行：验证流域 -> 率定 -> 训练期评估 -> 测试期评估 -> 总结报告。
...
- `hydroagent/skills/sensitivity_analysis/skill.md`
- `hydroagent/skills/sensitivity_analysis/tool.py`
...
```bash
python -m hydroagent -w results/exp_gr4j "率定GR4J模型，流域12025000"
python -m hydroagent -w results/exp_xaj  "率定XAJ模型，流域12025000"
```
